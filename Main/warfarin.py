...
model using a variational autoencoder (VAE) for the real Warfarin PK
dataset (O'Reilly 1968, PK-only -- PD/INR rows excluded).

Mirrors Main/tacrolimus.py's structure but is wired to:
  - functions_warfarin.load_warfarin: reads two source files
    (Warfarin_data.csv, Warfarin_covariates.csv), by default from this
    repo's own Data/ directory. These originate from the same source
    CVAE-LASSO uses (cvae_lasso/Data_real/); a snapshot copy was committed
    here so this repo is runnable standalone from a fresh clone. If
    cvae_lasso/Data_real/Warfarin_*.csv is corrected/updated upstream, the
    Data/ copy here must be re-synced manually (no automatic link).
    Warfarin_data.csv is a wide format with 14 fixed nominal slots
    (0.5,1,1.5,2,3,6,9,12,24,36,48,72,96,120h), blank = not observed for
    that subject. VAE-NLME's ODE-based decoder needs no fixed grid, so
    load_warfarin drops each subject's blanks and compacts the remaining
    observations into a dense variable-length (time, conc) sequence padded
    via `lengths`, exactly like every other dataset in this codebase -- no
    mask-matrix handling needed here (that's only required by CVAE-LASSO's
    fixed-vector architecture).
  - VAE/decoder_warfarin.Decoder_warfarin: 1-compartment first-order
    absorption model with lag, z_dim=4, parameters [Tlag, ka, Cl, V] --
    matches the PK-relevant subset of the parameters Ayral et al. 2021
    (COSSAC paper, Table 1) report for their joint "Warfarin PK/PD" model,
  - functions_warfarin.initalize_C / EmpiricalBayesEstimate.

Ground truth / structural model notes
--------------------------------------
Real covariates (n_true_cov = 3, per Ayral 2021 Table 1's "3 - SEX, logAGE,
logWT"): AGE, SEX, WT. Warfarin_covariates.csv now also carries 250 TCGA
RNAseq noise genes after these 3, so --n_cov can sweep past 3.

DOSE is computed on the fly as 1.5 * WT per subject (not read from a stored
value) -- matches the original recorded per-subject doses in the raw NONMEM
file almost exactly (ratio 1.4991-1.5007 across all 32 subjects).

Population priors [Tlag, ka, Cl, V] below are literature-informed
(warfarin's long elimination half-life ~20-60h implies a small Cl/V ratio;
V~8L and observed peak concentrations are consistent with typical
70kg-adult single-oral-dose warfarin PK) but are NOT user-confirmed ground
truth, since this is real (not simulated) data.
"""
#########################################################
# Import
#########################################################
...
from functions_warfarin import load_warfarin, initalize_C, EmpiricalBayesEstimate
from VAE.decoder_warfarin import Decoder_warfarin
...
parser = argparse.ArgumentParser(description="VAE-nlme Warfarin PK case study")
parser.add_argument('--data_dir', default=pk_data_dir_default,
                    help="Directory containing Warfarin_data.csv / Warfarin_covariates.csv")
...
parser.add_argument('--n_cov', type=int, default=None,
                    help="Use only the first n_cov covariate columns (default: all -- the "
                         "first 3 are the true covariates AGE/SEX/WT, the rest are "
                         "RNAseq-noise columns).")
...
# Load Data
#########################################################
data_path = os.path.join(args.data_dir, 'Warfarin_data.csv')
cov_path  = os.path.join(args.data_dir, 'Warfarin_covariates.csv')

(data, data_in, lengths, dose, weight_pop, covariates, covariates_in,
 covariate_names, n_cov) = load_warfarin(data_path, cov_path)
...
nbatch = data.shape[0]
x_dim = 2
z_dim = 4  # [Tlag, ka, Cl, V]
N_TRUE = 3  # AGE, SEX, WT (Ayral 2021 Table 1)
...
#########################################################
h_dim = 25
# Population priors, order [Tlag, ka, Cl, V] -- literature-informed
# magnitudes for oral warfarin (Tlag~0.5h, ka~1.0/h, Cl~0.15 L/h, V~8L; see
# module docstring), NOT user-confirmed ground truth.
mu0    = torch.tensor([0.5, 1.0, 0.15, 8.0])
...
#########################################################
Decoder = lambda z_normal, time, h: Decoder_warfarin(z_normal, time, h, dose)
...
print('')
print('#############################################')
print('ESTIMATION OF THE POPULATION PARAMETERS (Warfarin)')
print('#############################################')
z_pop_h = h(z_pop)
...
if args.results_json:
    ...
    with open(args.results_json, 'w') as f:
        json.dump(dict(
            dataset='warfarin', n_batch=nbatch, n_cov=n_cov, z_dim=z_dim, seed=args.seed,
